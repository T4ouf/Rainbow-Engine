...
const char* LevelEditor::m_objectsAvailable[] = { "Start", "Finish", "Cube", "Boxe", "Sphere" };
const char* LevelEditor::m_materialsAvailable[] = { "Gold", "Ruby", "Chrome", "Pearl", "Copper", "Obsidian", "BlackRubber", "LampMaterial" };
...
LevelEditor::LevelEditor() : Rainbow::Application(){
	//std::cout << "Level Editor !\n";
	scene = new Rainbow::Scene();
	...
	Rainbow::Shader* lamp = new Rainbow::Shader("Ressources/Shaders/SourceLumiere.shader");

	LampMaterial = new Rainbow::Material(lamp, nullptr,
	...
	Rainbow::Light& l = scene->retrieveLight();
...
void LevelEditor::OnUpdate(float delta) {
...
void LevelEditor::createObjectWindow() {
...
void LevelEditor::listObjectsWindow() {
...
void LevelEditor::mousePositionWindow() {
...
void LevelEditor::backgroundColorWindow() {
...
void LevelEditor::levelNameWindow() {
...
void LevelEditor::fileGenerationSuccessWindow() {
...
void LevelEditor::musicFileWindow() {
	ImGui::Begin("Music");
...
void LevelEditor::startItemsImGuiRender() {
...
void LevelEditor::finishItemsImGuiRender() {
...
void LevelEditor::cubeItemsImGuiRender() {
...
void LevelEditor::boxeItemsImGuiRender() {
...
void LevelEditor::sphereItemsImGuiRender() {
...
void LevelEditor::startCreation() {
	m_startPlaced = true;
	m_startCube = Rainbow::ObjectFactory::CreateCube(glm::vec3(m_startX, m_startY, m_startZ), 1, 0, true, Ruby);
	scene->addObject(m_startCube);
}

void LevelEditor::finishCreation() {
	m_finishPlaced = true;
	m_finishCube = Rainbow::ObjectFactory::CreateCube(glm::vec3(m_finishX, m_finishY, m_finishZ), 1, 0, true, Ruby);
	scene->addObject(m_finishCube);
}

void LevelEditor::cubeCreation() {
	// Create Cube and add it to the Scene
	glm::vec3 pos = glm::vec3(m_cubeX, m_cubeY, m_cubeZ);
	Rainbow::Object* cube = Rainbow::ObjectFactory::CreateCube(pos, m_cubeSize, m_cubeMass, m_cubeIsAnchor, m_currentMaterialSelected);
...
void LevelEditor::boxeCreation() {
	// Create Cube and add it to the Scene
	glm::vec3 pos = glm::vec3(m_boxeX, m_boxeY, m_boxeZ);
	Rainbow::Object* boxe = Rainbow::ObjectFactory::CreateBoxe(pos, m_boxeSizeX, m_boxeSizeY, m_boxeSizeZ, m_boxeMass, m_boxeIsAnchor, m_currentMaterialSelected);
...
void LevelEditor::sphereCreation() {
...
void LevelEditor::generateXMLFile() {
	...
	outfile << "<Objects>\n";
	for (std::unordered_map<Rainbow::Object*, std::string>::iterator it = m_existingObjectsXML.begin(); it != m_existingObjectsXML.end(); it++) {
		outfile << it->second;
	}
	outfile << "</Objects>\n";
	outfile << "</Structure>\n";
...
void LevelEditor::processMouseAction() {
...
void LevelEditor::processJoystickInput() {
